    ...
    public class DemandLoading
    {
            ...
            bool currentUser)
        {
            ...
            using (ack)
            {
                ...
                using (rk)
                {
                    rk.SetValue("DESCRIPTION", name, Microsoft.Win32.RegistryValueKind.DWord);
        ...
        private static void RemoveDemandLoadingEntries(bool currentUser)
        {
            try
            {
                ...
                RegistryKey ack = hive.OpenSubKey(HostApplicationServices.Current.UserRegistryProductRootKey);
                using (ack)
                {
